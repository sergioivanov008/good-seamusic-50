...
export const AudioPlayer = ({ tracks }: AudioPlayerProps) => {
  ...
  const [currentTimeFormatted, setCurrentTimeFormatted] = useState(TEXT.StartTime);
    ...
    if (audioRef.current) {
      ...
      const durSeconds = Math.floor(audioRef.current.duration % 60);
			if (isNaN(durMinutes) || isNaN(durSeconds)) {
				...
				const formattedTime =
				  `${durMinutes < 10 ? `0${durMinutes}` : durMinutes} : ${durSeconds < 10 ? `0${durSeconds}` : durSeconds}`;
  ...
  return (
		<div className={s.audioWrapper}>
			<audio src={tracks[trackIndex].trackUrl} ref={audioRef} onEnded={handlePlayNextTrack} onTimeUpdate={handleAudioUpdate} />
		  <div className={s.audioPlayer}>
              ...
              onChange={handleVolumeChange}
							style={{ "--curVolume": `${volume * 100}%` } as React.CSSProperties}
              className={s.volume}
            />
					</div>
            ...
            max={isNaN(duration) ? '1' : duration}
						...
						style={{ "--durProgress": `${(currentTime / duration) * 100}%` } as React.CSSProperties}
