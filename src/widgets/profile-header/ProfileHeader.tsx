...
const PROFILE_TEMP_DATA = {
  ...
  text: {
    name: 'Sam Mattal',
    login: '@sammattmusic',
    contentDescription: 'Hey, I’m Sam—an artist blending indie vibes with the grooves of house and deep house. I create music to move both your heart and your feet. Let’s vibe together 🤙',
  },
};
	...
	return (
		<div className={s.profileHeader}>
          ...
          <div className={s.profileBtns}>
            {PROFILE_TEMP_DATA.profileBtns.map((el, index) => (
              <MainBtn key={index} variant={'blackWhite'}>{el}</MainBtn>
            ))}
          </div>
          <div className={s.profileInfo}>
            {PROFILE_TEMP_DATA.profileInfo.map((el, index) => (
              <div key={index} className={s.profileInfoItem}>{el}</div>
            ))}
          </div>
        </div>
        ...
        <div className={s.userInfo}>
          ...
          <div className={s.infoContent}>
            <div className={s.contentDescription}>{PROFILE_TEMP_DATA.text.contentDescription}</div>
      ...
      <div className={s.navigation}>
            {PROFILE_TEMP_DATA.navigationLink.map((el, index) => (
              <div key={index} className={s.navigationLink}>{el}</div>
            ))}
      </div>
		</div>
	);
